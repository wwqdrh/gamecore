...
5. **程序化动画**（AnimGraph 类 + anim 模块）
   - AnimGraph：基于 anim-kit 库的数据驱动骨骼动画系统
   - 支持 JSON 配置加载，7 种运动基元（正弦波、三角波、方波、噪声、弹簧阻尼、脉冲、步行周期）
   - 支持风力、速度因子等全局参数运行时调整
   - 支持 IK 解算器（FABRIK、CCD、LookAt）
   - 每帧自动更新骨骼姿态，GDScript 按骨骼名获取变换
   - Juice 动画系统：移植自 C++ juice 库，基于 Godot Tween 的 UI 动画效果（波浪、旋转、抖动、弹跳、爆炸、收集、入场、呼吸、走路、打击标签、弹窗、点击反馈、Tooltip 等）
   - Easing 缓动枚举（31种），映射到 Godot 内置 TransitionType/EaseType
   - 防重入机制，防止同一动画重复触发
...
10. **UI 标记语言**（ui 模块）
    ...
    - TextureButton 支持：GML 中使用 `<TextureButton>` 标签，文字通过子 `<Label>` 元素添加（自动配置锚点填满、文字居中、鼠标穿透），样式系统中 texture 属性加载纹理到 texture_normal，支持 texture_normal/texture_pressed/texture_hover/texture_disabled 属性直接指定纹理路径
    - NinePatchRect 按钮模式：GML 中使用 `<NinePatchRect>` 标签加 `on_pressed` 属性实现九宫格拉伸按钮，自动添加不可见 Button 子节点处理点击事件，子 Label 自动配置锚点填满/文字居中/鼠标穿透，`patch_margin` 属性设置九宫格边距（支持 "10" 四边相同或 "10 5 10 5" left/top/right/bottom），class style texture 属性设置纹理
...
│       └── anim/           # 动画模块
│           ├── mod.rs      # 模块入口
│           ├── easing.rs   # Easing 缓动枚举（31种）
│           ├── juice.rs    # 核心动画系统（移植自C++ juice库）
│           ├── easy_move.rs # EaseMover 平滑移动
│           └── transition.rs # TransitionFade 过渡效果
│       └── state/           # 状态管理模块
│           ├── mod.rs       # 模块入口
...
Juice 动画系统移植自 C++ juice 库，提供丰富的 UI 动画效果。所有动画基于 Godot Tween 系统，使用 `tween_property` + 内置缓动类型。由于 gdext 0.5 的 `tween_method` 不支持 Rust 闭包，所有动画均使用 `tween_property` 实现。

### 核心机制

- **防重入机制**：每个动画函数内部使用 `check_is_handle`/`mark_handle` 检查和标记动画状态（基于 Godot meta），防止同一动画重复触发
- **清理回调**：动画完成后通过 `make_cleanup_callable`（使用 `Callable::from_fn`）清除 meta 标记
- **Easing 映射**：`easing_to_trans` 将自定义 Easing 枚举映射为 Godot `TransitionType`，`easing_to_ease_type` 映射为 `EaseType`

### 公开动画函数

| 函数 | 签名 | 说明 | 缓动类型 |
|------|------|------|----------|
| `anim_wave_simple` | `(target: &Gd<Node>, wave_offset: Vector2, duration: f64) -> Option<Gd<Tween>>` | 波浪摆动（循环） | SINE/IN_OUT |
| `anim_rotate_circle_simple` | `(target: &Gd<Node>, duration: f64) -> Option<Gd<Tween>>` | 360度旋转（循环） | LINEAR |
| `anim_shake_simple` | `(target: &Gd<Node>, duration: f64) -> Option<Gd<Tween>>` | 左右抖动（循环） | SINE/IN_OUT |
| `anim_move_straight_simple` | `(target: &Gd<Node>, to_pos: Vector2, duration: f64, ease_type: TransitionType) -> Option<Gd<Tween>>` | 直线移动 | 自定义 |
| `anim_bounce_simple` | `(target: &Gd<Node>, dire_pos: Vector2, distance: f32, duration: f64) -> Option<Gd<Tween>>` | 弹跳（弹出后返回） | BACK/OUT |
| `do_scale_simple` | `(target: &Gd<Node>, target_scale: Vector2, duration: f64) -> Option<Gd<Tween>>` | 缩放 | BACK/OUT |
| `anim_breath_simple` | `(target: &Gd<Node>, breath_factor: f32, anim_span: f64, duration: f64) -> Option<Gd<Tween>>` | 呼吸缩放（循环） | SINE/IN_OUT |
| `anim_walk_simple` | `(target: &Gd<Node>, walk_span: f64, duration: f64) -> Option<Gd<Tween>>` | 走路摆动（循环，旋转+缩放） | SINE/IN_OUT |
| `anim_enter` | `(target: &Gd<Node>, container: Option<Gd<Control>>, direction: i32, duration: f64, delay: f64, mode: i32) -> Option<Gd<Tween>>` | 入场动画（从界面外移入+透明度变化，按分组依次执行） | — |
| `anim_explosion` | `(target: &Gd<Node>, max_distance: f32, duration: f64) -> Option<Gd<Tween>>` | 爆炸散开+淡出（子节点动画） | BACK/IN |
| `anim_collect` | `(target: &Gd<Node>, target_position: Vector2, is_global: bool, duration: f64) -> Option<Gd<Tween>>` | 收集汇聚（子节点动画） | SINE/IN_OUT |
| `hit_label` | `(target: &Gd<Node>, duration: f64) -> Option<Gd<Tween>>` | 打击标签（缩放+颜色闪烁+上浮淡出） | BACK/OUT |
| `popup_enter` | `(target: &Gd<Control>, duration: f64) -> Option<Gd<Tween>>` | 弹窗弹入 | BACK/OUT |
| `popup_exit` | `(target: &Gd<Control>, duration: f64) -> Option<Gd<Tween>>` | 弹窗弹出 | SINE/IN |
| `click_feedback` | `(target: &Gd<Control>) -> Option<Gd<Tween>>` | 点击缩放反馈 | BACK/OUT |
| `tooltip_fade_in` | `(target: &Gd<Control>, duration: f64) -> Option<Gd<Tween>>` | Tooltip淡入 | SINE/OUT+BACK/OUT |
| `tooltip_fade_out` | `(target: &Gd<Control>, duration: f64) -> Option<Gd<Tween>>` | Tooltip淡出 | SINE/IN |

### 参数说明

- `direction`（anim_enter）：0=从下往上, 1=从上往下, 2=从左往右, 3=从右往左
- `mode`（anim_enter）：0=按 `anim_enter_` 前缀分组, 1=按子节点顺序逐个分组
- `is_global`（anim_collect）：true 使用 global_position，false 使用 position
- `duration`（anim_breath_simple/anim_walk_simple）：<= 0 表示无限循环
- 所有函数返回 `Option<Gd<Tween>>`，`None` 表示动画被防重入机制阻止或目标不支持该属性

### Rust 使用示例

```rust
use crate::anim::{anim_wave_simple, anim_bounce_simple, click_feedback};

// 波浪摆动动画
let tween = anim_wave_simple(&node, Vector2::new(0.0, 5.0), 0.5);

// 弹跳动画
let tween = anim_bounce_simple(&node, Vector2::new(100.0, 0.0), 50.0, 0.3);

// 点击反馈（Control 节点）
let tween = click_feedback(&control);
```
...
| `anim_enter` | 入场动画方向（bottom/top/left/right），同级兄弟自动递增延迟（stagger） |
| `anim_hover` | 悬停缩放倍数（如 "1.08"），"true" 默认 1.08，鼠标进入放大+提亮，离开恢复 |
| `anim_click` | 点击反馈动画（"true" 启用），点击时缩小再弹回 |
...
GdGmlScene 是一个继承 Control 的 GML 文件加载节点，位于 `rust/src/ui/ui_gml_scene.rs`。设置 `gml_file` 属性即可加载 .gml 文件并显示为 Control 节点树。信号自动连接到 GdGmlScene 自身脚本，可创建继承 GdGmlScene 的 GDScript 定义回调方法。构建完成后自动扫描 `anim_enter`/`anim_hover`/`anim_click` 属性并配置动画。
...
| 2026-06-14 | rust/src/anim/juice.rs | 完全重写：移除 callable_from_fn 和 tween_method+闭包模式，改用 tween_property + Godot 内置 TransitionType/EaseType；简化公开函数签名（移除 Easing 参数）；新增 easing_to_trans/easing_to_ease_type 映射函数；清理回调改用 Callable::from_fn；属性参数 &StringName→&NodePath，final_val Variant→&Variant |
| 2026-06-14 | rust/src/anim/transition.rs | 修复 gdext 0.5 兼容：移除 tween.bind()（create_tween 已自动绑定），StringName→NodePath，添加 easing_to_trans/easing_to_ease_type 缓动映射 |
| 2026-06-14 | rust/src/anim/easy_move.rs | 新建 EaseMover 平滑移动工具，移植自 C++ juice/easy_move |
| 2026-06-14 | rust/src/ui/ui_popup_panel.rs | 添加弹入/弹出动画：show_popup 缩放 0.85→1.0 (BACK/OUT) + 淡入 + 遮罩淡入；hide_popup 缩放 1.0→0.85 (SINE/IN) + 淡出 + 遮罩淡出；新增 animation_duration 和 anim_tween 字段 |
| 2026-06-14 | rust/src/ui/ui_tooltip.rs | 添加淡入淡出动画：show_tooltip 淡入 + 缩放 0.95→1.0 (BACK/OUT)；hide_tooltip 淡出 + 缩放 1.0→0.95 (SINE/IN)；新增 fade_duration 和 anim_tween 字段 |
| 2026-06-14 | rust/src/ui/ui_hlist.rs | 添加点击缩放反馈：handle_click 中缩放 0.9→1.0 (BACK/OUT) |
| 2026-06-14 | rust/src/ui/ui_vlist.rs | 添加点击缩放反馈：点击时缩放 0.9→1.0 (BACK/OUT) |
| 2026-06-14 | rust/src/ui/ui_grid.rs | 添加点击缩放反馈：移动端和 PC 端点击时缩放 0.9→1.0 (BACK/OUT) |
