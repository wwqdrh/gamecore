// GdMapIsometric - 等距双网格地图节点（继承 TileMapLayer）
// 提供等距（Isometric）双网格地形过渡、噪声地图生成、资源配置等封装方法
// 与 GdMapBasic 的区别：
//   - 显示层偏移为 (0, -0.5) * tile_size（等距网格），而非 (-0.5, -0.5) * tile_size（方形网格）
//   - 四角邻域方向为上/右/左/下，而非左上/右上/左下/右下
//   - 使用独立的等距查找表
// 内部结构：TileMapLayer (GdMapIsometric, 自身即世界层, self_modulate 透明)
//   ├── DisplayLayer_xxx (TileMapLayer, 等距偏移, 每种地形一个, 支持 priority 排序)
...
use super::dual_grid_iso::{
    DualGridIso, IsoTerrainType, IsoTerrainRegistry, IsoTerrainThresholds, IsoTerrainThresholdEntry,
    IsoPropConfig, iso_place_props,
};

/// 资源配置（从 JSON 加载）
#[derive(Debug, Clone)]
struct IsoResourceConfig {
    /// 地形配置：地形名 → (atlas_coord, source_id)
    terrain_atlas: HashMap<String, (Vector2i, i32)>,
    /// 显示层配置：地形名 → (source_id, priority)
    display_layers: HashMap<String, IsoDisplayLayerConfig>,
    /// 资源配置列表
    props: Vec<IsoPropConfig>,
}

/// 显示层配置
#[derive(Debug, Clone)]
struct IsoDisplayLayerConfig {
    /// TileSet source_id
    source_id: i32,
    /// 渲染优先级，数值越大越在上面
    priority: i32,
...
pub struct GdMapIsometric {
    base: Base<TileMapLayer>,

    /// 等距双网格算法核心
    dual_grid: DualGridIso,

    /// 地形注册表
    terrain_registry: IsoTerrainRegistry,

    /// 资源配置
    resource_config: Option<IsoResourceConfig>,
...
#[godot_api]
impl ITileMapLayer for GdMapIsometric {
    fn init(base: Base<TileMapLayer>) -> Self {
        Self {
            base,
            dual_grid: DualGridIso::new(),
            terrain_registry: IsoTerrainRegistry::new(),
            resource_config: None,
            display_layers: Vec::new(),
            prop_layer: None,
            can_set_tile: true,
            thresholds: IsoTerrainThresholds::default(),
        }
    }

    fn ready(&mut self) {
        // 自身即世界层，设置 self_modulate 为透明（alpha=0）
...
impl GdMapIsometric {
    ...
    fn get_terrain_name(&self, terrain_id: i32) -> GString {
        let id = IsoTerrainType::from_i32(terrain_id);
        self.terrain_registry
            .get_name(id)
    ...
    fn load_resource_config(&mut self, json_path: String) -> bool {
        let Ok(file) = std::fs::read_to_string(&json_path) else {
            godot_error!("GdMapIsometric: 无法读取配置文件: {}", json_path);
            return false;
        };

        let Ok(json) = serde_json::from_str::<serde_json::Value>(&file) else {
            godot_error!("GdMapIsometric: JSON 解析失败: {}", json_path);
            return false;
        };
        ...
        let config = match Self::parse_resource_config(&json, &mut self.terrain_registry) {
            Ok(c) => c,
            Err(e) => {
                godot_error!("GdMapIsometric: 配置解析失败: {}", e);
                return false;
            }
        };
    ...
    fn load_resource_config_from_string(&mut self, json_string: String) -> bool {
        let Ok(json) = serde_json::from_str::<serde_json::Value>(&json_string) else {
            godot_error!("GdMapIsometric: JSON 解析失败");
            return false;
        };
    ...
    fn set_terrain(&mut self, coords: Vector2i, terrain_type: i32) {
        ...
        let pos = (coords.x, coords.y);
        let terrain = IsoTerrainType::from_i32(terrain_type);
    ...
    fn generate_map(&mut self, width: i32, height: i32, seed: i64) {
        ...
        let terrains = DualGridIso::generate_terrain_from_noise(
    ...
    fn generate_map_from_tiles(&mut self) {
        ...
        let mut terrains: HashMap<(i32, i32), IsoTerrainType> = HashMap::new();
        let mut max_x = 0i32;
        let mut max_y = 0i32;
    ...
    fn clear_map(&mut self) {
        ...
        self.dual_grid = DualGridIso::new();
    }
    ...
    fn set_thresholds(
        ...
        for (name, max_val) in terrain_names.as_slice().iter().zip(threshold_maxs.as_slice().iter()) {
            if let Some(id) = self.terrain_registry.get_id(&name.to_string()) {
                entries.push(IsoTerrainThresholdEntry {
                    terrain_id: id,
                    max_value: *max_val,
                });
            }
        }
        self.thresholds = IsoTerrainThresholds { entries };
    }
    ...
    fn add_terrain_config(
        ...
        if self.resource_config.is_none() {
            self.resource_config = Some(IsoResourceConfig {
        ...
        if let Some(ref mut config) = self.resource_config {
            ...
            config.display_layers.insert(terrain_name, IsoDisplayLayerConfig {
    ...
    fn add_prop_config(
        ...
        if let Some(ref mut config) = self.resource_config {
            let terrains: Vec<IsoTerrainType> = allowed_terrains
                .as_slice()
                .iter()
                .filter_map(|s| {
            ...
            config.props.push(IsoPropConfig {
                name: name.to_string(),
                source_id,
                alternative_tile,
                probability,
...
impl GdMapIsometric {
    ...
    fn parse_resource_config(
        ...
        registry: &mut IsoTerrainRegistry,
    ) -> Result<IsoResourceConfig, String> {
        ...
        if let Some(layers) = json.get("display_layers").and_then(|l| l.as_object()) {
            for (name, value) in layers {
                ...
                let priority = value
                    ...
                    .and_then(|v| v.as_i64())
                    .unwrap_or(0) as i32;
                display_layers.insert(name.clone(), IsoDisplayLayerConfig {
                    source_id,
                    priority,
                });
            }
        ...
        if let Some(props_arr) = json.get("props").and_then(|p| p.as_array()) {
            for prop_val in props_arr {
                ...
                let probability = prop_val
                    ...
                    .and_then(|v| v.as_f64())
                    .unwrap_or(0.05);
                let allowed_terrains: Vec<IsoTerrainType> = prop_val
    ...
    fn ensure_display_layers(&mut self) {
        let sorted_layers: Vec<(String, IsoDisplayLayerConfig)> = if let Some(ref config) = self.resource_config {
            let mut entries: Vec<(String, IsoDisplayLayerConfig)> = config.display_layers
                .iter()
            ...
            self.terrain_registry.get_all_names()
                .into_iter()
                .enumerate()
                .map(|(i, name)| (name, IsoDisplayLayerConfig {
                    source_id: 0,
                    priority: i as i32,
                }))
                .collect()
    ...
    fn sync_tile_set_to_layers(&mut self) {
        ...
        if let Some(ref ts) = tile_set {
            ...
            // 等距网格偏移：仅 Y 方向偏移半格
            let offset_y = tile_size.y as f32 / 2.0;

            for layer in &self.display_layers {
                let mut layer = layer.clone();
                layer.set_tile_set(ts);
                // 等距显示层偏移：X 不偏移，Y 偏移半格
                layer.set_position(Vector2::new(0.0, -offset_y));
            }
            if let Some(ref mut layer) = self.prop_layer {
                layer.set_tile_set(ts);
            }
    ...
    fn atlas_to_terrain(&self, atlas: (i32, i32)) -> IsoTerrainType {
    ...
    /// 获取地形在世界层的 source_id
    fn get_terrain_source_id(&self, terrain: IsoTerrainType) -> i32 {
        if let Some(ref config) = self.resource_config {
    ...
    fn get_terrain_atlas_and_source(&self, terrain: &IsoTerrainType) -> ((i32, i32), i32) {
        if let Some(ref config) = self.resource_config {
            if let Some(name) = self.terrain_registry.get_name(*terrain) {
    ...
    /// 获取地形在显示层的 source_id
    fn get_display_source_id(&self, terrain: IsoTerrainType) -> i32 {
        if let Some(ref config) = self.resource_config {
            if let Some(name) = self.terrain_registry.get_name(terrain) {
    ...
    fn refresh_all_display_tiles(&mut self) {
        ...
        // 第一轮：先设置占位地块，确保所有 display position 都有基础 tile
        // 避免后续 calculate_display_tile 读取 corner 时因缺少占位而得到错误结果
        let sorted_layers = self.get_sorted_display_layer_info();
        for &display_pos in &all_display_positions {
            self.base_mut().set_cell_ex(Vector2i::new(display_pos.0, display_pos.1))
                .source_id(0)
                .atlas_coords(Vector2i::new(0, 0)) // 默认全背景占位
                .done();
        }

        // 第二轮：正确计算每个 display tile
        for display_pos in all_display_positions {
            self.update_single_display_tile(display_pos);
        }
    }
    ...
    fn place_props_from_config(
        ...
        let placements = iso_place_props(
            max_x + 1,
            max_y + 1,
            noise_values,
            terrains,
